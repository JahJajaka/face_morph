...
if __name__ == "__main__":
	...
	if args.folder:
		...
		if not os.path.exists(imgFolder):
			raise ValueError(f"Folder '{imgFolder}' does not exist")
		listImg = sorted(os.listdir(imgFolder))
	else:
		raise ValueError(f"folder is not specified")
	...
	if not args.beats:
		raise ValueError(f"Beats file is not specified")

	if not os.path.exists(args.beats):
		raise ValueError(f"Beats file doesn't exist")
	...
	for i in range(beat_segments):
		...
		video_path = args.tmpfolder + "segment_" + str(i) + "_" + args.output
		print(video_path)
